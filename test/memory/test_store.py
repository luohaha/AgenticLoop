...
class TestMessageStorage:
    ...
    async def test_save_message_with_complex_content(self, store):
        ...
        await store.save_message(session_id, msg, tokens=20)
...
class TestToolCallsSerialization:
    ...
    async def test_save_assistant_message_without_tool_calls(self, store):
...
class TestBatchSave:
    ...
    async def test_save_memory(self, store):
        ...
        # Save all at once
        await store.save_memory(session_id, system_messages, messages, summaries)
...
class TestSummaryStorage:
    ...
    async def test_save_summary(self, store):
